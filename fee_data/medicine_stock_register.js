...
// View Switcher (Dashboard vs Student Matrix vs Official Stock Voucher)
        ...
        card.innerHTML = `
            <div>
                ...
                <div class="stock-meter-row">
                    ...
                    <span class="${isLow ? 'text-amber-600 font-bold' : 'text-emerald-700 font-bold'}">
                        ${stock.available} / ${stock.received} ${med.unit || 'पीस'}
                    </span>
                </div>

                <div class="stock-progress-bar">
                    <div class="stock-progress-fill ${isLow ? 'low-stock' : ''}" style="width: ${percent}%;"></div>
                </div>
                ...
                <div class="med-stock-numbers">
                    ...
                    <div>
                        ...
                        <div class="mini-stat-val ${isLow ? 'text-red-600' : 'text-emerald-600'}">${stock.available}</div>
                    </div>
                </div>
            </div>
            ...
            <tr style="${isChecked ? 'background: #eff6ff;' : ''}">
                ...
                <td>
                    ...
                    <div class="text-xs text-slate-500">
                        PEN/Roll: <span class="font-semibold text-slate-700">${st.admissionNo}</span>
...
function populateMedicineSelect() {
    ...
    if (medSelect) {
        medSelect.innerHTML = `<option value="">-- दवाई चुनें --</option>`;
        medicines.forEach(med => {
            const stock = getMedicineStock(med.id);
            medSelect.innerHTML += `<option value="${med.id}">${med.nameHi} (${med.nameEn || ''}) [उपलब्ध: ${stock.available} ${med.unit || 'पीस'}]</option>`;
        });
    }
            ...
            <tr>
                <td colspan="8">
                    <div class="empty-state">
                        ...
                        <p class="font-bold text-slate-700">कोई वितरण रिकॉर्ड उपलब्ध नहीं है।</p>
                        <p class="text-xs text-slate-500 mt-1">ऊपर दिए गए "+ नई दवाई वितरित करें" बटन पर क्लिक करके छात्र को दवाई वितरित करें।</p>
                    </div>
                </td>
            </tr>
        `;
